...
| Загрузить контент в CSV файле             | GET    | Можно         | —                   | При экспорте **RQL-ограничение limit(...) принудительно снимается обработчиком CSV**, поэтому возвращается **весь** контент датастора. С 12.0.0 вывод соответствует RFC 4180 (`""` для экранирования кавычек) и начинается со строки заголовков (имена колонок), — см. [CHANGELOG §12.0.0](CHANGELOG.md). | **Требуется** заголовок `download` со значением `csv`                               | В данный момент ситуация с `limit` это баг и о нём известно. Требуемый заголовок также в дальнейшем будет изменен согласно стандартам (`Accept: text/csv`). Экспорт CSV — не санитизируется от формульной инъекции: не отдавайте выгрузки неавторизованным пользователям. |
