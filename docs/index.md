...
обновляться). Для DbTable операция выполняется в одной транзакции. Возвращает список идентификаторов успешно обновленных записей;
...
Массовое обновление выполняется PATCH-запросом с заголовком `X-DataStore-Operation: multi-update`. Операция атомарная —
если одна строка не проходит валидацию, изменения откатываются.
...
Ответ: массив идентификаторов обновленных записей. Проверьте поддержку операции по заголовку `X_MULTI_UPDATE` в ответе
на HEAD-запрос к ресурсу. При ошибке (валидация, ограничения БД) сервер отвечает кодом `4xx/5xx`, изменения не сохраняются.
...
- `X-DataStore-Operation` - управляет специальными операциями. Для массового обновления укажите `X-DataStore-Operation: multi-update`
  и передайте JSON-массив записей.
...
- `X_MULTI_UPDATE` - заголовок, который свидетельствует поддержки транзакционного multiUpdate
