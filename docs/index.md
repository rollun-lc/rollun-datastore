...
- `multiUpdate($records)` - обновляет несколько существующих записей в одной атомарной транзакции. Если запись не
существует, она пропускается. При любой ошибке выполняется rollback всей транзакции. Возвращает список
идентификаторов успешно обновленных записей. **Требует реализации метода** в DataStore (не имеет fallback);
...
- **Требуется реализация метода `multiUpdate()`** в DataStore
...
- Используется оптимизированный SQL запрос с `UPDATE ... CASE WHEN` для DbTable
- Записи блокируются через `SELECT ... FOR UPDATE` для предотвращения race conditions
- Если запись не существует, она пропускается (не выбрасывается исключение)
- Записи без идентификатора пропускаются
- Обновляются только существующие поля в каждой записи
...
| 500 | Общая ошибка БД | `{"error": "Can't multi update records..."}` |
...
- `X_MULTI_UPDATE` - заголовок, который свидетельствует поддержки multiUpdate
